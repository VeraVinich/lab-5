...
ids = [obj for obj in data if re.fullmatch(r'\d+', obj)] 
fam = [obj for obj in data if re.fullmatch(r'[A-Z][a-zA-Z]+', obj)]
emails = [obj for obj in data if re.fullmatch(r'\S+@\S+', obj)]
date = [obj for obj in data if re.fullmatch(r'\d{4}-\d{2}-\d{2}', obj)]
web = [obj for obj in data if re.fullmatch(r'https?://\S+|www\.\S+', obj)]
